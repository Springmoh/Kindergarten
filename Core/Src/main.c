...
void StartDefaultTask(void *argument) {
	...
	for (;;) {
		if (!startup) {
			...
			if (HAL_GetTick() - resettimer1 >= RSTTIME) { // i put 10 seconds only here for demo
				flag1 = 0;
				resettimer1 = HAL_GetTick();
			}

			if (HAL_GetTick() - resettimer2 >= RSTTIME) { // i put 10 seconds only here for demo
				flag2 = 0;
				resettimer2 = HAL_GetTick();
			}

			if (HAL_GetTick() - resettimer3 >= RSTTIME) { // i put 10 seconds only here for demo
				flag3 = 0;
				resettimer3 = HAL_GetTick();
			}
